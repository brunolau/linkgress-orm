...
- `pnpm test:pglite` (`tests-pglite/run-jest-suite.js`) — the whole jest suite on PGlite: sets `LINKGRESS_TEST_DRIVER=pglite` and starts jest under `--experimental-vm-modules`, on half the cores unless a worker option is passed (every file owns its PGlite; the config's `maxWorkers: 1` exists for the shared server database). ~24 s vs ~148 s serially on the server, same outcomes as a serial PGlite run. PGlite `import()`s its WASM/data bundles, which jest's vm context only allows with that flag; `globalSetup` refuses to start without it, and `tests/database/pglite-client.test.ts` skips itself.
- `globalSetup` builds the AppDatabase schema once (plus `pg_trgm`/`unaccent`, which a long-lived test server has from earlier runs) and `dumpDataDir()`s it to a temp tarball (`LINKGRESS_TEST_PGLITE_SNAPSHOT`); every test file boots its OWN instance from it (`tests/utils/pglite-server.ts`, ~150 ms). Shared and fresh harness clients borrow that instance; `disposeSharedDatabase()` stops it. Without a snapshot (runners that skip globalSetup) the server builds the schema itself behind a gate.
- Files that construct `PgClient`/`PostgresClient` directly still hit the real server; on pglite, globalSetup only warns when it is unreachable.
- Expected on PGlite (measured in `bench/pglite/README.md`): 11 failures, all attributed — 5 specs that seed through the harness but query their own `PostgresClient`, the 4 `query-batch-fidelity` date-revival tests (fail on `LINKGRESS_TEST_DRIVER=postgres` too), `for-update` (needs two sessions; the single-session guard fails it at once) and `collation` (no ICU data). PGlite's database collation is `C` and its session `TimeZone` a fixed offset (`Etc/GMT-1`).
- `LINKGRESS_TEST_RECORD_DIR=<dir>` records every harness statement with a result digest (`tests/utils/query-recorder.ts`); `node bench/pglite/compare-runs.mjs --a <pg.json> --b <pglite.json> --a-rec <dir> --b-rec <dir>` diffs two runs: outcomes, per-file timings, statement results.
