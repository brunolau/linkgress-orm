...
Multi-level navigation properties (2+ levels) now work correctly in grouped query chains. Previously, patterns like `p.product.category.timezone` or `ot.task.level.createdBy.email` inside a `.select().groupBy().select()` chain would crash with "cannot read property X of undefined" during query building.
...
const result = await db.productPrices
  .select(pp => ({
    ...
    timezone: pp.product!.category!.timezone, // 2-level navigation
    price: pp.price,
  }))
  .groupBy(p => ({
