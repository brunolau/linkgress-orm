...
```typescript
// Before: Crashed with "Cannot read properties of undefined (reading 'level')"
...
const inserted = await db.orderTasks
  ...
  .returning(ot => ({
    id: ot.id,
    // Collection accessed through multi-level navigation: orderTask -> task -> level -> createdBy
