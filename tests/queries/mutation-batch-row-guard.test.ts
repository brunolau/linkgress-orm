...
describe('MutationBatch rowGuard', () => {
  ...
  test('a $N inside a -- comment in the guard is legal', async () => {
    await withDatabase(async (db) => {
      const { users } = await seedTestData(db);

      // Comments are their own token class in QUOTED_OR_PLACEHOLDER, so a $N
      // sitting in guard prose is not a bare placeholder. Pins that the
      // tripwire reads the placeholder group and not a neighbouring one — the
      // scan is positional-index-free for exactly this reason. Also pins that
      // the leg's trailing `-- …` does not swallow the fused CTE's RETURNING,
      // which sits on the next line.
      const batch = new MutationBatch();
      const key = batch.addInsertBulk(db.posts, [
        {
          title: 'Commented guard',
          content: 'x',
          userId: users.charlie.id,
          views: 0,
          publishTime: { hour: 12, minute: 0 },
        },
      ], 'guarded', { rowGuard: 'v."views" >= 0 -- the cap was $1 before' });
